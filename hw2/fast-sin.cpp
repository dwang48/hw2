...
void sin4_taylor(double* sinx, const double* x) {
  for (int i = 0; i < 4; i++) {
    ...
    s += x9  * c9;
    s += x11 * c11;
    if (-0.25*PI<=x1 && x1<=0.25*PI)
    {
    sinx[i] = s;
    }
    if (0.25*PI<x1  && x1 <=0.75*PI)
    {
    sinx[i] = pow((1-s*s),0.5);
    }
    if (0.75*PI<x1  && x1 <=1.25*PI)
    {
    sinx[i] = -s;
    }
    if (1.25*PI<x1  && x1 <=1.75*PI)
    {
    sinx[i] = -pow((1-s*s),0.5);
    }
...
int main() {
  ...
  for (long i = 0; i < N; i++) {
    x[i] = (drand48()-0.5) * PI/2; // [-pi/4,pi/4]
    sinx_ref[i] = 0;
    sinx_taylor[i] = 0;
    sinx_intrin[i] = 0;
    sinx_vector[i] = 0;
...
//Extra Credit Explanation: basically, what I did was to use if condition to divide the sin(x) function into 4 parts,
//[-pi/4,pi/4],(pi/4,3pi/4],(3pi/4,5pi/4],(5pi/4,7pi/4).
//Since we know e^ix = cos(x)+isin(x) and every 90 degrees shift was to multiply e^ix by i, 
//I obtained 4 formulae for each of the 4 parts. We only look at the imaginary parts
//of the formulae and we can use taylor expansions of sin(x),cos(x),-sin(x),-cos(x) 
//to do approximation in each of the 4 parts. We can use the taylor expansion of sin(x) in the part
//of sin(x) and sin(-x). For cos(x) and -cos(x), we can use Pythagoras theorem.
